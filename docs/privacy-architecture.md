...
The gallery-dl adapter reads browser cookies at runtime. It ignores external gallery-dl configuration, scopes Chromium/Firefox cookie reads to the active built-in site's domain (`.x.com` for X and `.instagram.com` for Instagram), disables cookie export and persistent gallery-dl caching, and does not print raw extraction failures. X Likes and Bookmarks share one process and its in-memory cookie cache. Authenticated X/Instagram work uses a hashed site + browser-session coordination key so concurrent ClipBox processes do not duplicate the same site/session operation; profile paths or cookie values are not written into the lock filename. Preview execution plans are memory-only and expire after 10 minutes. Process stdin/stdout/stderr use concurrently drained memory pipes, not temporary text files. The process arguments contain no cookie values.
