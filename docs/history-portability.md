...
SQLite is planned as the authoritative local archive database. Stable IDs are stored as text.

The canonical migration format will use the `.clipboxbackup` extension. It is intended to be a versioned, self-describing archive that can be restored or merged on another computer.
